...
export function defaultParams() {
  return {
    ...
    loaderChase: 0.85,
    loaderBottom: 1, // vh
    loaderOut: 0.25, // seconds to fade once the hold lets go
    ...
    // 0.4 starts the move midway through the ring drawing.
    stageAt: 0.4,
    stageAt: 0.45,
    spinTurns: 1,
    spinTime: 0.9,
    ...
    dragSpeed: 1,
    scrollSpeed: 0.001, // rad/s of angular velocity per px of wheel delta (calm, controllable)
    damping: 0.92, // velocity kept per 60fps frame
    maxSpeed: 5, // rad/s, capped so flicks never run away
    dragSpeed: 0.75,
    ...
    snapTime: 1.2, // gentle, premium settle time
    snapFrom: 0.8, // rad/s under which the ring commits to a slot
    pickTime: 0.95, // click-to-centre: seconds for one slot, root-scaled
    pickEase: "power3.out",
